...
tests/
├── __init__.py
├── conftest.py              # 共享 fixtures（CLI/执行链/样例数据）
...
旧 Python Web/API 的历史集成测试已经迁到：

```text
legacy/web_api/tests/
```

这些测试不再属于 `worker-py` 的默认执行链，也不属于生产部署必需部分。

## 编写新测试

1. 新增执行链相关测试放在 `tests/integration/` 或 `tests/unit/`
